...
typedef struct 
{
    uint8_t gw_task_indx;
} uart_ctrl_st;


void uart_func_task(void);
void uart_gw_task(void);
...
atask_st gw_handle             = {"UART Gateway   ", 100,0, 0, 255, 0, 1, uart_gw_task};
...
void uart_initialize(void)
{
    ...
    uctrl.gw_task_indx =  atask_add_new(&gw_handle);
...
void uart_parse_rx_frame(void)
{
    ...
    if (do_continue)
    {
        ...
        uart.rx.msg.action      = (uart_action_et)uart.rx.msg.str.charAt(5);
        #ifdef DEBUG_PRINT
...
bool uart_build_node_from_rx_str(void)
{
    // <A1M1:Piha;PIR1;L;->
    bool    is_ok = true;
    int     indx1=0;
    int     indx2=0;
    uint8_t len = uart.rx.msg.str.length();
    uart.rx.msg.str.trim();
    if (len < 8) is_ok = false;
    if (is_ok){
        if((uart.rx.msg.str.charAt(0) != '<') &&
            (uart.rx.msg.str.charAt(len-1) != '>')) 
        {
            is_ok = false;
        }
    } 
    if (is_ok){
        ...
        indx1 = uart.rx.msg.str.indexOf('<') + 1;
        indx2 = uart.rx.msg.str.indexOf(':');       
        if((indx1 > 0) && (indx2 > indx1)){
            uart.node.addr = uart.rx.msg.str.substring(indx1,indx2);
        } else is_ok = false;
    } 
    if(is_ok){
        indx1 = indx2+1; 
        indx2 = uart.rx.msg.str.indexOf(';', indx1);
        if((indx1 > 0) && (indx2 > indx1)){
...
void uart_print_node(void)
{
    Serial.print("Node: ");
    Serial.print(uart.node.addr);
    Serial.print(" - "); Serial.print(uart.node.zone);
    Serial.print(" - "); Serial.print(uart.node.name);
    Serial.print(" - "); Serial.print(uart.node.value);
    Serial.print(" - "); Serial.println(uart.node.remark);
  }
...
void uart_gw_task(void)
{
    switch(gw_handle.state)
    {
        case 0:
            ...
            break;
        case 10:
            uart.tx.msg.str = "<A1D1?->";
            SerialGw.println(uart.tx.msg.str);
            Serial.println(uart.tx.msg.str);
            uart.rx.timeout = millis() + 2000;
            gw_handle.state = 20;
            break;
        case 20:
            if (SerialGw.available()) {
...
void uart_alarm_handling_task(void)
{
    switch(uart_alarm_handle.state)
    {
        case 0:
            ...
            uart.tx.msg.action = UART_ACTION_GET;
            uart.tx.msg.value = "-";
